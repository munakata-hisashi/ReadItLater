...
## TestFlight配信メモ
`/Users/hisashi/work/Claude-work/ios-dev-tools` の `xcloud-trigger` を使ってXcode Cloudを手動起動します。

- PR向け配信ワークフロー（feature/claude/codexブランチ向け）: `PR TestFlight` / `68A68B9F-ADBD-464D-AC49-30C771F25948`
- main向け配信ワークフロー（自動実行）: `TestFlight Beta` / `8FDBF6AD-8DC4-45CA-91E7-31ADB05419AB`

実行手順（手動配信）:
1. `cd /Users/hisashi/work/Claude-work/ios-dev-tools`
2. 配信対象ブランチを指定して起動（`<branch-name>`だけ差し替え）
   `swift run xcloud-trigger --workflow-id "68A68B9F-ADBD-464D-AC49-30C771F25948" --branch "<branch-name>" --key-id "N2J27YRD3B" --private-key-path "/Users/hisashi/keys/AuthKey_N2J27YRD3B.p8" --issuer-id "281db7e9-96e8-478b-a973-fbea7429c7f8"`
3. 出力された `build_run_id` で状態確認
   `TOKEN=$(swift run asc-token --key-id "N2J27YRD3B" --private-key-path "/Users/hisashi/keys/AuthKey_N2J27YRD3B.p8" --issuer-id "281db7e9-96e8-478b-a973-fbea7429c7f8" | head -n 1); curl -s -H "Authorization: Bearer $TOKEN" "https://api.appstoreconnect.apple.com/v1/ciBuildRuns/<build_run_id>"`
